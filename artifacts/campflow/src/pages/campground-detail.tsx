...
export default function CampgroundDetail() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([listCampgrounds(), isAuthenticated ? listFavorites() : Promise.resolve({ data: [] })])
      .then(([campgroundsData, favoritesData]) => {
        setCampgrounds(campgroundsData);
        setFavoriteIds(favoritesData.data.map((item) => String(item._id ?? item.id)));
        if (campground) {
          return listCampsites(String(campground._id));
        }
        return Promise.resolve([] as ApiItem[]);
      })
      .then((campsites) => {
        setSites(campsites);
      })
  ...
  useEffect(() => {
    if (campground) {
      setLoading(true);
      listCampsites(String(campground._id))
        .then(setSites)
        .catch(() => setError('We could not load campsites for this campground.'))
        .finally(() => setLoading(false));
    }
  }, [campground]);

  const campsiteTypes = useMemo(() => {
  ...
  const typeCounts = useMemo(() => {
    ...
    sites.forEach((site) => {
      const type = String(site.type ?? 'unknown');
  ...
  const handleToggleFavorite = async (site: ApiItem) => {
    ...
    const id = String(site._id ?? site.id);
    try {
  ...
  const selectedSite = sites.find((site) => String(site._id ?? site.id) === selectedSiteId);

  if (loading) {
    return <div className="space-y-16 pb-10 md:space-y-24"><p className="text-sm text-muted-foreground">Loading campground…</p></div>;
  }

  if (error || !campground) {
    return <div className="space-y-16 pb-10 md:space-y-24"><p className="text-sm text-destructive">{error ?? 'Campground not found.'}</p></div>;
  }
  ...
  return <div className="space-y-16 pb-10 md:space-y-24">
    ...
    <section><div className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">Park map</p><h2 className="mt-3 font-serif text-4xl tracking-tight">Where to stay</h2></div><Badge variant="secondary">{sites.length} available sites</Badge></div><div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]"><div className="space-y-4"><div className="grid gap-4 sm:grid-cols-2">{sites.map((site) => { const siteId = String(site._id ?? site.id); const isFavorite = favoriteIds.includes(siteId); return <Card key={siteId}><CardContent className="p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">{site.type ?? 'Site'}</p><h3 className="mt-2 text-xl font-semibold">{site.name ?? `Site ${siteId}`}</h3></div><Button size="sm" variant={isFavorite ? 'secondary' : 'outline'} disabled={savingFavorite} onClick={() => void handleToggleFavorite(site)}>{isFavorite ? 'Saved' : 'Save'}</Button></div><p className="mt-4 text-sm leading-6 text-muted-foreground">{site.description ?? 'A comfortable place to unwind by the fire.'}</p></CardContent></Card>; })}</div></div><div><InteractiveCampMap campgroundName={campground.name} campgroundId={String(campground._id ?? campground.id)} /></div></div></section>
  </div>;
}
